// ===== ToLife! v1.0 — App Logic =====
// L'Chaim — to health, to strength
...
let data = {
  weights: [], workouts: [], checkins: [], calories: [], myFoods: [], startDate: null
};
...
function loadData() {
  ...
  if (saved) { const p = JSON.parse(saved); data = { ...data, ...p }; if (!data.myFoods) data.myFoods = []; }
...
function showPage(name) {
  ...
  if (name === 'Weight') { updateMetabolicDisplay(); renderWeightHistory(); renderWeightStats(); }
  if (name === 'Workout') renderWorkoutHistory();
...
function updateSlider(type) {
  const map = {
    ...
    rpe: { id: 'rpeSlider', valId: 'rpeVal', colors: ['var(--accent-teal)','var(--accent-teal)','var(--accent-green)','var(--accent-green)','var(--accent-blue)','var(--accent-blue)','var(--accent-orange)','var(--accent-orange)','var(--accent-red)','var(--accent-red)'] }
  };
...
function setActivity(level) {
  ...
  document.getElementById('calDetail').textContent = 'TDEE ' + calcTDEE(w, level).toLocaleString() + ' − ' + CONFIG.deficitPerDay + ' deficit (with 5% adjustments)';
...
function saveCheckin() {
  ...
  data.checkins.push({ date: today(), stress, sleep: currentSleep, energy, note });
  const w = getCurrentWeight(); const target = calcTarget(w, currentActivity);
  const todayLog = getTodayFoodLog(); const consumed = todayLog.reduce((s, f) => s + f.calories, 0);
  data.calories = data.calories.filter(c => c.date !== today());
  data.calories.push({ date: today(), activity: currentActivity, target, consumed, adherence: currentAdherence, log: todayLog });
  saveData(); showToast('✅ Check-in saved!'); refreshDashboard();
}
...
function saveWorkout() {
  if (!currentWorkoutType) { showToast('⚠️ Select a workout type'); return; }
  data.workouts.push({
    date: today(), type: currentWorkoutType,
    rpe: currentWorkoutType !== 'rest' ? parseInt(document.getElementById('rpeSlider').value) : 0,
  ...
  showToast('💪 Workout logged!'); renderWorkoutHistory(); refreshDashboard();
}

function renderWorkoutHistory() {
  const c = document.getElementById('workoutHistory');
  const recent = [...data.workouts].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 15);
  if (!recent.length) { c.innerHTML = '<div style="padding:12px;color:var(--text-muted);font-size:0.85rem">No workouts yet.</div>'; return; }
  const badges = { weights:'badge-weights', cardio:'badge-cardio', stretch:'badge-stretch', rest:'badge-rest' };
  const labels = { weights:'🏋️ Weights', cardio:'🚴 Cardio', stretch:'🧘 Stretch', rest:'🛋 Rest' };
  c.innerHTML = recent.map((w, i) => {
    const ex = []; if (w.duration) ex.push(w.duration+'min'); if (w.hr) ex.push('❤️'+w.hr); if (w.focus) ex.push(w.focus==='fatburn'?'🔥 fat burn':'💓 intensity');
    return '<div class="log-entry"><span class="log-date">'+formatDate(w.date)+'</span><div class="log-detail"><span class="log-badge '+badges[w.type]+'">'+labels[w.type]+'</span><span style="font-size:0.8rem;color:var(--text-secondary)">'+(ex.length?' · '+ex.join(' · '):'')+'</span>'+(w.note?'<div style="font-size:0.75rem;color:var(--text-muted);margin-top:2px">'+w.note+'</div>':'')+'</div>'+(w.type!=='rest'?'<span class="log-rpe" style="color:var(--accent-purple)">RPE '+w.rpe+'</span>':'')+'<button class="delete-btn" onclick="deleteWorkout('+i+')" title="Delete">×</button></div>';
  }).join('');
}
...
function deleteWorkout(di) {
  ...
  if (idx > -1) { data.workouts.splice(idx, 1); saveData(); renderWorkoutHistory(); refreshDashboard(); showToast('🗑 Deleted'); }
}

// ===== WEIGHT =====
function saveWeight() {
  const w = parseFloat(document.getElementById('weightInput').value);
  if (!w || w < 100 || w > 400) { showToast('⚠️ Enter a valid weight'); return; }
  data.weights = data.weights.filter(e => e.date !== today());
  data.weights.push({ date: today(), weight: w }); saveData();
  document.getElementById('weightInput').value = '';
  showToast('⚖️ Weight: ' + w + ' lbs');
  updateMetabolicDisplay(); renderWeightHistory(); renderWeightStats(); renderWeightChart(); refreshDashboard();
}
...
function renderWeightHistory() {
  ...
  c.innerHTML = recent.map((e, i) => {
    ...
    return '<div class="log-entry"><span class="log-date">'+formatDate(e.date)+'</span><span style="font-family:JetBrains Mono;font-weight:700;font-size:0.95rem">'+e.weight.toFixed(1)+' lbs</span>'+ds+'<button class="delete-btn" onclick="deleteWeight(\''+e.date+'\')" title="Delete">×</button></div>';
  }).join('');
}

function deleteWeight(date) { data.weights = data.weights.filter(w => w.date !== date); saveData(); renderWeightHistory(); renderWeightStats(); renderWeightChart(); refreshDashboard(); showToast('🗑 Deleted'); }

function renderWeightStats() {
  const sorted = [...data.weights].sort((a, b) => a.date.localeCompare(b.date));
  if (sorted.length < 2) { document.getElementById('avg7day').textContent = '—'; document.getElementById('weeklyChange').textContent = '—'; return; }
...
async function estimateCalories() {
  ...
  if (!input) { showToast('⚠️ Describe what you ate'); return; }
  ...
  } finally { btn.disabled = false; btn.textContent = '🔍 Estimate'; }
}
...
async function callAI(food) {
  if (CONFIG.workerUrl) {
    ...
    } catch(e) { console.log('Worker unavailable'); }
  }
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages:[{role:'user',content:'Estimate calories for this food/meal. Return ONLY valid JSON, no markdown.\n\nFood: "'+food+'"\n\nReturn: {"items":[{"name":"item","calories":123,"protein":10,"carbs":15,"fat":5}],"total_calories":123,"total_protein":10,"total_carbs":15,"total_fat":5,"note":"portion note"}\n\nRules: standard servings, multiply calories by 1.05 for conservative adjustment, round to nearest 5, ONLY return JSON.'}]
      })
    });
    if (r.ok) { const d = await r.json(); const txt = d.content.map(c=>c.text||'').join(''); return JSON.parse(txt.replace(/```json|```/g,'').trim()); }
  } catch(e) { console.log('API unavailable'); }
  return estimateLocally(food);
}

function estimateLocally(desc) {
  const lower = desc.toLowerCase(); const items = []; let tC=0,tP=0,tCa=0,tF=0;
  const foods = {
    'cheerios':{name:'Cheerios (1 cup)',cal:110,p:3,c:22,f:2},'multigrain cheerios':{name:'Multigrain Cheerios (1 cup)',cal:110,p:3,c:24,f:1.5},
    'almond milk':{name:'Almond milk (1 cup)',cal:30,p:1,c:1,f:2.5},'oatmeal':{name:'Oatmeal (1 cup)',cal:160,p:6,c:27,f:3.5},
    'eggs':{name:'Eggs (2 large)',cal:144,p:12,c:1,f:10},'egg':{name:'Egg (large)',cal:72,p:6,c:0.5,f:5},
    'toast':{name:'Toast (1 slice)',cal:80,p:3,c:14,f:1},'bagel':{name:'Bagel',cal:270,p:10,c:53,f:1.5},
    'banana':{name:'Banana',cal:105,p:1.3,c:27,f:0.4},'apple':{name:'Apple',cal:95,p:0.5,c:25,f:0.3},
    'yogurt':{name:'Greek yogurt (1 cup)',cal:130,p:17,c:9,f:4},'coffee':{name:'Coffee with milk',cal:30,p:1,c:2,f:1.5},
    'chicken breast':{name:'Chicken breast (6oz)',cal:280,p:53,c:0,f:6},'chicken':{name:'Chicken (6oz)',cal:280,p:42,c:0,f:12},
    'salmon':{name:'Salmon (6oz)',cal:350,p:40,c:0,f:20},'steak':{name:'Steak (6oz)',cal:380,p:46,c:0,f:20},
    'tuna':{name:'Tuna can',cal:200,p:40,c:0,f:2},'protein shake':{name:'Protein shake',cal:160,p:30,c:5,f:2},
    'protein bar':{name:'Protein bar',cal:220,p:20,c:25,f:8},'rice':{name:'Rice (1 cup)',cal:210,p:4,c:45,f:0.5},
    'pasta':{name:'Pasta (1 cup)',cal:220,p:8,c:43,f:1.3},'bread':{name:'Bread (1 slice)',cal:80,p:3,c:14,f:1},
    'potato':{name:'Potato',cal:160,p:4,c:37,f:0.2},'sweet potato':{name:'Sweet potato',cal:115,p:2,c:27,f:0.1},
    'salad':{name:'Salad with dressing',cal:180,p:4,c:12,f:14},'broccoli':{name:'Broccoli (1 cup)',cal:55,p:3.7,c:11,f:0.6},
    'almonds':{name:'Almonds (1/4 cup)',cal:210,p:8,c:7,f:18},'nuts':{name:'Mixed nuts (1/4 cup)',cal:200,p:6,c:8,f:17},
    'cheese':{name:'Cheese (1 oz)',cal:110,p:7,c:0.5,f:9},'peanut butter':{name:'Peanut butter (2 tbsp)',cal:190,p:8,c:7,f:16},
    'sandwich':{name:'Sandwich',cal:400,p:22,c:40,f:18},'burger':{name:'Hamburger',cal:550,p:30,c:40,f:30},
    'pizza':{name:'Pizza (2 slices)',cal:560,p:22,c:62,f:24},'soup':{name:'Soup (1 bowl)',cal:180,p:10,c:20,f:6},
    'wrap':{name:'Wrap/burrito',cal:450,p:25,c:45,f:18},'juice':{name:'Juice (8oz)',cal:120,p:1,c:28,f:0},
    'wine':{name:'Wine (5oz)',cal:125,p:0,c:4,f:0},'beer':{name:'Beer (12oz)',cal:150,p:1,c:13,f:0}
  };
  let matched = false;
  // Check multi-word keys first (longest match)
  const sortedKeys = Object.keys(foods).sort((a,b) => b.length - a.length);
  for (const key of sortedKeys) {
    if (lower.includes(key)) {
      const f = foods[key]; const adj = Math.round(f.cal * CONFIG.intakeAdjust);
      items.push({name:f.name,calories:adj,protein:f.p,carbs:f.c,fat:f.f});
      tC+=adj; tP+=f.p; tCa+=f.c; tF+=f.f; matched=true;
    }
  }
  if (!matched) {
  ...
  return {items,total_calories:tC,total_protein:Math.round(tP),total_carbs:Math.round(tCa),total_fat:Math.round(tF),note:'Estimated locally. Configure AI worker for more accurate results.'};
}

function displayAiResult(r) {
  const d = document.getElementById('aiResult');
  let h = '';
  r.items.forEach(i => { h += '<div class="ai-result-item"><span class="food-name">'+i.name+'</span><span class="food-cal">'+i.calories+' cal</span></div>'; });
  h += '<div class="ai-result-total"><span>Total (+5% adjustment)</span><span class="total-cal">'+r.total_calories+' cal</span></div>';
  h += '<div class="ai-result-macro"><span>🥩 Protein: '+r.total_protein+'g</span><span>🍞 Carbs: '+r.total_carbs+'g</span><span>🧈 Fat: '+r.total_fat+'g</span></div>';
  if (r.note) h += '<div class="ai-result-note">'+r.note+'</div>';
  h += '<div class="ai-result-actions"><button class="btn-primary" onclick="logAiFood()" style="font-size:0.85rem;padding:10px">✅ Log This</button><button class="btn-secondary" onclick="saveToMyFoods()" style="margin:0;font-size:0.85rem;padding:10px">⭐ Save to My Foods</button></div>';
...
function logAiFood() {
  ...
  document.getElementById('aiResult').classList.remove('visible'); lastAiResult = null;
  renderFoodLog(); updateCalorieSummary(); showToast('🍽 Food logged!');
}
...
function saveToMyFoods() {
  ...
  saveData(); renderMyFoods(); showToast('⭐ Saved to My Foods!');
}
...
function renderMyFoods() {
  ...
  if (!data.myFoods.length) {
    c.innerHTML = '<div style="padding:12px;color:var(--text-muted);font-size:0.85rem">No saved foods yet. Use AI estimator and tap "Save to My Foods" to build your list.</div>';
    document.getElementById('addSelectedBtn').style.display = 'none'; return;
  }
  const sorted = [...data.myFoods].sort((a,b) => (b.count||0) - (a.count||0));
  c.innerHTML = sorted.map(f =>
    '<div class="my-food-item" id="mf-'+f.id+'" onclick="toggleMyFood(\''+f.id+'\')"><div class="my-food-check" id="mfc-'+f.id+'">✓</div><div class="my-food-info"><div class="my-food-name">'+f.name+'</div><div class="my-food-cal">'+f.calories+' cal · P:'+f.protein+'g C:'+f.carbs+'g F:'+f.fat+'g'+(f.count?' · used '+f.count+'×':'')+'</div></div><button class="my-food-remove" onclick="event.stopPropagation();removeMyFood(\''+f.id+'\')" title="Remove">×</button></div>'
  ).join('');
...
function toggleMyFood(id) {
  ...
  btn.textContent = '✅ Add ' + selectedMyFoods.size + ' Selected to Today\'s Log';
}
...
function addSelectedMyFoods() {
  ...
  selectedMyFoods.forEach(id => {
    ...
    if (food) {
      entry.log.push({id:generateId(),name:food.name,calories:food.calories,protein:food.protein,carbs:food.carbs,fat:food.fat,time:new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'})});
  ...
  showToast('🍽 ' + count + ' item(s) added!');
}

function removeMyFood(id) {
  if (confirm('Remove from My Foods?')) {
    data.myFoods = data.myFoods.filter(f => f.id !== id); saveData(); renderMyFoods(); showToast('🗑 Removed');
  }
}
...
function renderFoodLog() {
  ...
  c.innerHTML = log.map(f =>
    '<div class="food-log-item"><span class="food-log-name">'+f.name+' <span style="color:var(--text-muted);font-size:0.75rem">'+(f.time||'')+'</span></span><span class="food-log-cal">'+f.calories+'</span><button class="food-log-remove" onclick="removeFoodLogItem(\''+f.id+'\')" title="Remove">×</button></div>'
  ).join('');
}
...
function removeFoodLogItem(id) {
  ...
  renderFoodLog(); updateCalorieSummary(); showToast('🗑 Removed');
}
...
function updateCalorieSummary() {
  ...
  document.getElementById('calRemaining').textContent = rem > 0 ? rem.toLocaleString()+' cal remaining' : Math.abs(rem).toLocaleString()+' cal over';
  ...
  document.getElementById('macroSummary').textContent = '🥩 '+tP+'g protein · 🍞 '+tCa+'g carbs';
}
...
function refreshDashboard() {
  ...
  document.getElementById('progressRing').style.strokeDashoffset = circ - (pct/100) * circ;
  ...
  document.getElementById('goalSubtitle').innerHTML = 'Week <strong>'+Math.min(weeksIn,CONFIG.goalWeeks)+'</strong> of '+CONFIG.goalWeeks+' — <strong>'+Math.max(0,toGo).toFixed(1)+'</strong> lbs to go';
  ...
  document.getElementById('dashStress').textContent = todayCheckin ? todayCheckin.stress+'/10' : '—';
  const todayW = data.workouts.filter(x=>x.date===today());
  document.getElementById('dashWorkout').textContent = todayW.length ? (todayW[todayW.length-1].type==='rest'?'Rest':todayW[todayW.length-1].rpe+'/10') : '—';
  const consumed = todayCal&&todayCal.log ? todayCal.log.reduce((s,f)=>s+f.calories,0) : 0;
  document.getElementById('dashConsumed').textContent = consumed ? consumed.toLocaleString() : '—';
  renderWeekGrid(); renderNudge(todayCheckin); renderWeightChart(); renderDashboardInsights();
}
...
function renderWeekGrid() {
  ...
  let h = '';
  for (let i=0;i<7;i++) { const d=new Date(sow); d.setDate(d.getDate()+i); const ds=d.toISOString().split('T')[0];
    const wo = data.workouts.find(w=>w.date===ds); const icons={weights:'🏋️',cardio:'🚴',stretch:'🧘',rest:'🛋'};
    h += '<div class="week-day '+(wo?'completed':'')+' '+(ds===today()?'today':'')+'"><div class="week-day-label">'+dn[i]+'</div><div class="week-day-icon">'+(wo?icons[wo.type]:(ds===today()?'·':''))+'</div></div>';
  }
  g.innerHTML = h;
}
...
function renderNudge(checkin) {
  ...
  const nudges = [
    ...
    'Consider stretching or mobility today — it reduces cortisol while keeping your streak alive.',
    'On high-stress days, even 15 min on the bike is a win. Don\'t skip — scale back instead.',
...
function renderDashboardInsights() {
  ...
  if (sorted.length >= 14) {
    ...
    const wl = pA - rA;
    if (wl<0.5) insights.push({type:'warn',icon:'⚠️',text:'Weight loss slowed to '+wl.toFixed(1)+' lbs/week. Consider increasing calorie adjustment to 10%.'});
    else if (wl>2.5) insights.push({type:'warn',icon:'⚡',text:'Losing '+wl.toFixed(1)+' lbs/week — consider eating more to preserve muscle.'});
  }
  const streak = calcStreak();
  if (streak>=7) insights.push({type:'good',icon:'🔥',text:streak+'-day streak! Consistency is the #1 predictor of success.'});
...
function renderCorrelations() {
  ...
  if (hi.length>=3) { const hw=hi.filter(x=>data.workouts.some(w=>w.date===x.date&&w.type!=='rest')); const sr=Math.round((1-hw.length/hi.length)*100);
    if (sr>40) ins.push({type:'warn',icon:'😤',text:'On high-stress days, you skip workouts '+sr+'% of the time. Even 15 min of easy cardio helps.'}); }
  const hc = hi.map(x=>data.calories.find(y=>y.date===x.date)).filter(Boolean);
  const pa = hc.filter(x=>x.adherence==='poor'||x.adherence==='ok');
  if (hc.length>=3&&pa.length/hc.length>0.5) ins.push({type:'warn',icon:'🍞',text:'On high-stress days, calorie adherence drops. Pre-plan protein-rich snacks.'});
  ...
  if (hfl.length>=2) { const ac=hfl.reduce((s,x)=>s+x.log.reduce((ss,f)=>ss+(f.carbs||0),0),0)/hfl.length; const ap=hfl.reduce((s,x)=>s+x.log.reduce((ss,f)=>ss+(f.protein||0),0),0)/hfl.length;
    if (ac>ap*2) ins.push({type:'warn',icon:'🍞',text:'On stressed days, carb-to-protein ratio is '+(ac/ap).toFixed(1)+':1. Try protein-first meals.'}); }
  ...
  if (ps.length>=3) { const ae=ps.reduce((s,x)=>s+x.energy,0)/ps.length; const aa=data.checkins.reduce((s,x)=>s+x.energy,0)/data.checkins.length;
    if (ae<aa-0.5) ins.push({type:'info',icon:'😴',text:'Poor sleep drops energy by '+(aa-ae).toFixed(1)+' points. Sleep may be the highest-leverage change.'}); }
  if (lo.length>=3) { const lw=lo.filter(x=>data.workouts.some(w=>w.date===x.date&&w.rpe>=6)); if (lw.length/lo.length>0.6) ins.push({type:'good',icon:'💪',text:'On low-stress days you push harder (RPE 6+). Keep capitalizing!'}); }
  c.innerHTML = ins.length ? ins.map(i=>'<div class="insight-card insight-'+i.type+'"><span class="insight-icon">'+i.icon+'</span><span>'+i.text+'</span></div>').join('') : '<div class="insight-card insight-info"><span class="insight-icon">📊</span><span>Keep logging daily for sharper pattern insights.</span></div>';
}

function renderAdaptiveSuggestions() {
  const c = document.getElementById('adaptiveSuggestions');
  if (data.weights.length<14) { c.innerHTML='<div class="insight-card insight-info"><span class="insight-icon">📊</span><span>Keep logging — after 2-3 weeks ToLife! will suggest adjustments.</span></div>'; return; }
  ...
  if (wl<0.5) sug.push({type:'warn',icon:'🎯',text:'Plateau detected. Try: increase cal adjustment to 10%, add one extra cardio session, or reduce portion at largest meal.'});
  else if (wl>=1&&wl<=2) sug.push({type:'good',icon:'✅',text:'Losing '+wl.toFixed(1)+' lbs/week — right on track!'});
  else if (wl>2.5) sug.push({type:'warn',icon:'⚡',text:'Losing '+wl.toFixed(1)+' lbs/week. Add 200 cal/day to protect muscle.'});
...
function exportData() { const b=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(b); a.download='tolife-backup-'+today()+'.json'; a.click(); URL.revokeObjectURL(a.href); showToast('📤 Exported!'); closeDataModal(); }
function importData(ev) { const f=ev.target.files[0]; if(!f) return; const r=new FileReader(); r.onload=e=>{try{const im=JSON.parse(e.target.result); if(im.weights&&im.workouts){data={myFoods:[],...im};saveData();showToast('📥 Imported!');refreshDashboard();closeDataModal();}}catch{showToast('⚠️ Error');}}; r.readAsText(f); ev.target.value=''; }
function resetAllData() { if(confirm('⚠️ Delete ALL data?')&&confirm('Really? Cannot be undone.')){ data={weights:[],workouts:[],checkins:[],calories:[],myFoods:[],startDate:today()}; saveData(); showToast('🗑 Reset'); closeDataModal(); refreshDashboard(); } }

// ===== INIT =====
function init() {
  loadTheme(); loadData();
  const w=getCurrentWeight(); document.getElementById('calTarget').textContent=calcTarget(w,'moderate').toLocaleString();
  document.getElementById('calDetail').textContent='TDEE '+calcTDEE(w,'moderate').toLocaleString()+' − '+CONFIG.deficitPerDay+' deficit (with 5% adjustments)';
  ...
  if(tca){currentActivity=tca.activity||'moderate';if(tca.adherence)currentAdherence=tca.adherence;}
